...
module grid
  ...
  real(kind=dp),dimension(0:mesh) :: x !< spatial cooridnate   (was r)
!  real(kind=dp),dimension(:),allocatable :: x !< spatial coordinate x (not used)
  real(kind=dp),dimension(mesh) :: y !< spatial cooridnate   (was r)
  real(kind=dp),dimension(mesh) :: z !< spatial cooridnate   (was r)
...
contains
  ...
  subroutine grid_ini()
    ...
    if (rank == 0) then
       ...
       write(logf,*) 'The inner radius is ', r_in, ' cm'
       write(logf,*) 'The outer radius is ', r_out, ' cm'
    endif
