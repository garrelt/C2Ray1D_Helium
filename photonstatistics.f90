...
MODULE photonstatistics
  ...
  USE material, ONLY: ndens, xHI, xHII, xHeI, xHeII, xHeIII, temper, clumping !xh, xhe
...
CONTAINS
  ...
  SUBROUTINE state_before ()
    ...
    he2_before=0.0
    DO i=1,mesh
       h0_before=h0_before+vol(i)*ndens(i,1,1)*xHI(i)*(1.0_dp-abu_he)
       h1_before=h1_before+vol(i)*ndens(i,1,1)*xHII(i)*(1.0_dp-abu_he)
       he0_before=he0_before+vol(i)*ndens(i,1,1)*xHeI(i)*abu_he
       he1_before=he1_before+vol(i)*ndens(i,1,1)*xHeII(i)*abu_he
       he2_before=he2_before+vol(i)*ndens(i,1,1)*xHeIII(i)*abu_he
  ...
  SUBROUTINE total_rates(dt)
    ...
    DO i=1,mesh
       ...
       yhe(1)=xHeII(i)
       yhe(2)=xHeIII(i)
       totrec=totrec+vol(i)*ndens(i,1,1)* electrondens(ndens(i,1,1),yh(1),yhe(1),yhe(2))*clumping* &
          (xHII(i)*(1.0_dp-abu_he)*  & 
	  ...
	  xHeIII(i)*abu_he*  &
       ...
       totcollisions=totcollisions+vol(i)*ndens(i,1,1)*(1.0_dp-abu_he)* &
            xHI(i)*electrondens(ndens(i,1,1),yh(1),yhe(1),yhe(2))* &
            colh0*SQRT(temper(i))*EXP(-temph0/temper(i))+ &
	    vol(i)*ndens(i,1,1)*abu_he*   &
            xHeI(i)*electrondens(ndens(i,1,1),yh(1),yhe(1),yhe(2))* &
            colhe(0)*SQRT(temper(i))*EXP(-temphe(0)/temper(i))+ &
	    vol(i)*ndens(i,1,1)*abu_he*   &
            xHeII(i)*electrondens(ndens(i,1,1),yh(1),yhe(1),yhe(2))* &
  ...
  SUBROUTINE state_after()
    ...
    DO i=1,mesh
       h0_after=h0_after+vol(i)*ndens(i,1,1)*xHI(i)*(1.0_dp-abu_he)
       h1_after=h1_after+vol(i)*ndens(i,1,1)*xHII(i)*(1.0_dp-abu_he)
       he0_after=he0_after+vol(i)*ndens(i,1,1)*xHeI(i)*abu_he
       he1_after=he1_after+vol(i)*ndens(i,1,1)*xHeII(i)*abu_he
       he2_after=he2_after+vol(i)*ndens(i,1,1)*xHeIII(i)*abu_he
    ENDDO
