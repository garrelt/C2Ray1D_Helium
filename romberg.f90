...
module romberg
  ...
  real(kind=dp),dimension(0:2**maxpow, -1:maxpow) :: romw_for_pl_scaling !< Romberg weights
...
contains
  ...
  SUBROUTINE romberg_initialisation (nmax,nmax_for_pl_scaling)
    ...
    integer,intent(in) :: nmax_for_pl_scaling
    ...
    if (nmax /= 2 ** pmax) stop &
         'ROMBERG-Error : # grid points has to be a power of 2 (non pl scaling)'
    ...
    pmax = nint(log (real(nmax_for_pl_scaling,dp))/log(2.0))
    if (pmax > maxpow) stop  &
         'ROMBERG-Error : Too many grid points'
    if (nmax_for_pl_scaling /= 2 ** pmax) stop &
         'ROMBERG-Error : # grid points has to be a power of 2 (pl scaling)'
    !					Set up extrapolation constants
    do k = 1, pmax
       b (k) = -1.0 / (4.0 ** k - 1.0)
       a (k) = - b (k) * 4.0 ** k
    ...
    do k = 0, pmax
       ...
       do i = k, pmax
          do j = 0, 2 ** k
             romw_for_pl_scaling(2**(i-k)*j,i)=s(i,i)*2**(i-k)+romw_for_pl_scaling(2**(i-k)*j,i)
          enddo
       enddo
       s (k,0) = 0.0
    enddo
    romw_for_pl_scaling (0, -1) = 1.0
    ! Weights at the edge have to be halved
    do i = 0, pmax
       romw_for_pl_scaling (0, i) = 0.5 * romw_for_pl_scaling (0, i)
       romw_for_pl_scaling (2**i, i) = 0.5 * romw_for_pl_scaling (2**i, i)
    enddo
	
	
    write(*,*) 'romw',sum(romw)		
    write(*,*) 'romw_for_pl_scaling',sum(romw_for_pl_scaling)	
  ...
  FUNCTION Scalar_Romberg_for_pl_scaling (f, w, nc, nx, ny) result(scalar_romberg_result)
    ...
    do y = 0, ny
       do x = 0, nx
          integral = integral + f (x, y) * w (x, y) * &
               romw_for_pl_scaling (x, px) * romw_for_pl_scaling (y, py)
       enddo
    enddo
	
    scalar_romberg_result = integral
    
  end FUNCTION Scalar_Romberg_for_pl_scaling
